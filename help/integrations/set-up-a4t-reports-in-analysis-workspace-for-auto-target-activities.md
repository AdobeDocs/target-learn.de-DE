...
In diesem Artikel werden die empfohlenen Änderungen für die Analyse von [!DNL Auto-Target]-Aktivitäten in Workspace erläutert, die auf den folgenden Hauptkonzepten basieren:
...
* Besuche sollten als Normalisierungsmetrik verwendet werden, wenn Leistungsunterteilungen auf Erlebnisebene angezeigt werden. Darüber hinaus kann die Standardzählmethodik von [Adobe Analytics Besuche umfassen, bei denen der Benutzer keinen Aktivitäten-Inhalt](https://experienceleague.adobe.com/docs/target/using/integrate/a4t/a4t-faq/a4t-faq-viewing-reports.html?lang=en#metrics) tatsächlich sieht. Dieses Standardverhalten kann jedoch mithilfe eines entsprechend segmentgebundenen Segments (Details unten) geändert werden.
* Die Verwendung von Scoping-Zuordnungen mit Besuchsrückblick - auch als &quot;Besuchs-Lookback-Fenster&quot;im vorgeschriebenen Zuordnungsmodell bezeichnet - wird von den ML-Modellen von Adobe Target während der Schulungsphase verwendet, und dasselbe (nicht standardmäßige) Zuordnungsmodell sollte idealerweise bei der Unterteilung der Zielmetrik verwendet werden.

## A4T für [!DNL Auto-Target]-Bedienfeld in Workspace erstellen

Um einen A4T-Bericht für [!DNL Auto-Target] zu erstellen, verwenden Sie entweder den Beginn mit dem typischen Bereich &quot;Analytics für Zielgruppen&quot;in Workspace, wie unten dargestellt, oder beginnen Sie mit einer Freiformtabelle. Wählen Sie dann die folgenden Optionen aus:

1. **[!UICONTROL Kontrollerlebnis]**: Sie können ein beliebiges Erlebnis auswählen. Wir werden diese Entscheidung jedoch später überschreiben. Beachten Sie, dass das Kontrollerlebnis für [!DNL Auto-Target]-Aktivitäten eine Steuerungsstrategie ist, die entweder a) zufällig unter allen Erlebnissen dient oder b) ein einziges Erlebnis bereitstellt (diese Auswahl erfolgt zum Zeitpunkt der Erstellung der Aktivität in Adobe Target). Auch wenn Sie sich für Auswahl (b) entschieden haben - Ihre [!DNL Auto-Target]-Aktivität hat ein bestimmtes Erlebnis als Control bezeichnet - sollten Sie weiterhin den in diesem Artikel beschriebenen Ansatz zur Analyse von A4T für [!DNL Auto-Target]-Aktivitäten verfolgen.
1. **[!UICONTROL Normalisierungsmetrik]**: Wählen Sie Besuche.
1. **[!UICONTROL Erfolgsmetriken]**: Obwohl Sie Metriken auswählen können, für die Berichte erstellt werden sollen, sollten Sie in der Regel Berichte zu derselben Metrik erstellen, die bei der Erstellung der Aktivität in Adobe Target zur Optimierung ausgewählt wurde.
...
*pngAbbildung 2: Der empfohlene Basisbericht für  [!DNL Auto-Target] Aktivitäten. Dieser Bericht wurde so konfiguriert, dass zielgerichteter Traffic (vom Ensemble-ML-Modell bereitgestellt) mit dem Kontrolldatenverkehr* verglichen wird

>[!NOTE]
>
>Derzeit stehen für A4T-Berichte zur automatischen Zielgruppe keine Lift- und Konfidenzzahlen für &quot;Control&quot;- bzw. &quot;Targeting&quot;-Dimensionen zur Verfügung. Bis Support hinzugefügt wird, können Steigerung und Konfidenz manuell berechnet werden, indem der [Vertrauensrechner](https://experienceleague.adobe.com/docs/target/assets/complete_confidence_calculator.xlsx?lang=en) heruntergeladen wird.

## Aufschlüsselungen von Metriken auf Erlebnisebene
...
Wählen Sie bei der Analyse einer [!DNL Auto-Target]-Aktivität immer Besuche als Standard-Normalisierungsmetrik. [!DNL Auto-Target] personalization wählt ein Erlebnis für einen Besucher einmal pro Besuch aus (formell einmal pro Adobe Target-Sitzung), d. h. das Erlebnis, das einem Benutzer angezeigt wird, kann sich bei jedem einzelnen Besuch ändern. Wenn wir also eindeutige Besucher als Normalisierungsmetrik verwenden, kann es vorkommen, dass ein einzelner Benutzer mehrere Erlebnisse (über verschiedene Besuche hinweg) sieht, was zu verwirrenden Konversionsraten führen würde.
...
Wie aus der Tabelle hervorgeht, besteht eine eindeutige Inkongruenz von Zahlen auf Besucher-Ebene. Obwohl es insgesamt zwei individuelle Besucher gibt, ist dies keine Summe individueller Besucher für jedes Erlebnis. Obwohl der Konversionsrate auf Besucherebene nicht unbedingt falsch ist, machen Konversionsraten auf Besucherebene beim Vergleich einzelner Erlebnisse wohl wesentlich sinnvoller aus. Formal ist die Analyse (&quot;Besuche&quot;) dieselbe wie die Entscheidungs-Stickiness-Einheit, was bedeutet, dass Summen und Vergleiche von Aufschlüsselungen von Metriken auf Erlebnisebene sinnvoll sind.

## Erweitert: Methode zur Zählung von nicht standardmäßigen Besuchen
...
### So erstellen Sie das richtige Segment:

1. Wählen Sie in der Symbolleiste &quot;Arbeitsbereich&quot;die Option **[!UICONTROL Komponenten > Segment]** erstellen.
1. Geben Sie einen **[!UICONTROL Titel]** für Ihr Segment ein. Im folgenden Beispiel heißt das Segment [!DNL "Hit with specific Auto-Target activity"].
1. Ziehen Sie die Dimension **[!UICONTROL Zielgruppe Aktivitäten]** in den Bereich **[!UICONTROL Definition]**.
1. Verwenden Sie den Operator **[!UICONTROL equals]**.
1. Suchen Sie nach Ihrer spezifischen Aktivität der Zielgruppe.
1. Wählen Sie das Zahnradsymbol aus und wählen Sie **[!UICONTROL Zuordnungsmodell > Instanz]**, wie in der folgenden Abbildung dargestellt.
...
Nachdem das Segment erstellt wurde, können wir es verwenden, um die Besuchsmetrik zu filtern, sodass es nur Besuche enthält, bei denen der Benutzer mit der Zielgruppe-Aktivität interagiert hat.
...
1. Lassen Sie das Segment frei. Der Filter wird auf diese Metrik angewendet.
...
## Erweitert: Zuweisung zum Besuchsbereich für Zielmetriken

Die A4T-Integration ermöglicht, dass das ML-Modell von [!DNL Auto-Target] *trainiert* wird, indem dieselben Konversions-Ereignis-Daten verwendet werden, die zum Generieren von Performance-Berichten *in Adobe Analytics verwendet werden.* Es gibt jedoch bestimmte Annahmen, die bei der Interpretation dieser Daten bei der Schulung der ML-Modelle zugrunde gelegt werden müssen, die sich von den während der Berichte-Phase in Adobe Analytics gemachten Standardannahmen unterscheiden.
...
## Letzter Schritt: Konversionsraten mit besuchsbezogener Zuordnung und gefilterter Besuchsnormalisierung
...
1. Ziehen Sie Ihr zuvor erstelltes Segment mit dem Namen &quot;Treffer mit spezifischer [!DNL Auto-Target]-Aktivität&quot;in diesem Artikel für diese spezifische [!DNL Auto-Target]-Aktivität.
...
>[!IMPORTANT]
>
>Die Konversionsrat-Metrik aus dem A4T-Bedienfeld ist nicht mit dem Konversions-Ereignis oder der Normalisierungsmetrik in der Tabelle verknüpft. Wenn Sie die in diesem Artikel vorgeschlagenen Änderungen vornehmen, passt sich das Konversionsrate nicht an die Änderungen an. Wenn Sie die Änderung an einer (oder beiden) Konversionsmetrik und der Normalisierungsmetrik vornehmen, müssen Sie sich daher als letzten Schritt daran erinnern, das Konversionsrate ebenfalls zu ändern, wie oben gezeigt.

## Zusammenfassung: Endgültiges Beispiel für einen Arbeitsbereich für [!DNL Auto-Target]-Berichte

Die folgende Abbildung fasst alle oben genannten Schritte in einem Bedienfeld zusammen und zeigt eine vollständige Ansicht des empfohlenen Berichts für [!DNL Auto-Target] A4T-Aktivitäten. Dieser Bericht entspricht dem Bericht, den die Zielgruppe zur Optimierung Ihrer Zielmetrik für maschinelles Lernen verwendet, und enthält alle in diesem Artikel besprochenen Nuancen und Empfehlungen. Dieser Bericht steht auch den Zählmethodiken bei herkömmlichen Zielgruppe-Berichte-basierten [!DNL Auto-Target]-Aktivitäten am nächsten.
